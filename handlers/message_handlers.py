        ...
        try:
            ...
            if message.text:
                ...
                try:
                    ...
                    logger.error(f"转发未知类型消息失败: {e}")
                    await bot.send_message(
                        chat_id=GROUP_ID,
                        message_thread_id=topic_id,
                        text=f"⚠️ 收到不支持的消息类型，无法转发。"
                    )
    ...
    async def handle_owner_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        if update.effective_user.id in MessageHandlers.edit_states:
            ...
            try:
                ...
                if message.text:
                    ...
                    new_msg = await context.bot.send_location(
                        chat_id=user_id,
                        latitude=message.location.latitude,
                        longitude=message.location.longitude
                    )
                    ...
                    new_msg = await context.bot.send_animation(
                        chat_id=user_id,
                        animation=message.animation.file_id,
                        caption=message.caption
                    )
                    ...
                    new_msg = await context.bot.send_contact(
                        chat_id=user_id,
                        phone_number=message.contact.phone_number,
                        first_name=message.contact.first_name,
                        last_name=message.contact.last_name
                    ...
                    new_msg = await context.bot.send_dice(
                        chat_id=user_id,
                        emoji=message.dice.emoji
                    )
                    message_id = new_msg.message_id
                    ...
                    new_msg = await context.bot.send_venue(
                        chat_id=user_id,
                        latitude=message.venue.location.latitude,
                        longitude=message.venue.location.longitude,
                        title=message.venue.title,
                    ...
                    new_msg = await context.bot.send_video_note(
                        chat_id=user_id,
                        video_note=message.video_note.file_id
                    )
                    message_id = new_msg.message_id
        ...
        try:
            ...
            if message.text:
                ...
                forwarded_msg = await bot.send_location(
                    chat_id=user_id,
                    latitude=message.location.latitude,
                    longitude=message.location.longitude
                )
            elif message.animation:
                forwarded_msg = await bot.send_animation(
                    chat_id=user_id,
                    animation=message.animation.file_id,
                    caption=message.caption
                )
            elif message.contact:
                forwarded_msg = await bot.send_contact(
                    chat_id=user_id,
                    phone_number=message.contact.phone_number,
                    first_name=message.contact.first_name,
                    last_name=message.contact.last_name
                ...
                forwarded_msg = await bot.send_dice(
                    chat_id=user_id,
                    emoji=message.dice.emoji
                )
            elif message.venue:
                forwarded_msg = await bot.send_venue(
                    chat_id=user_id,
                    latitude=message.venue.location.latitude,
                    longitude=message.venue.location.longitude,
                    title=message.venue.title,
                ...
                forwarded_msg = await bot.send_video_note(
                    chat_id=user_id,
                    video_note=message.video_note.file_id
                )
            elif message.poll:
                ...
                try:
                    ...
                    logger.error(f"转发未知类型消息失败: {e}")
                    await message.reply_text(f"⚠️ 不支持的消息类型，无法转发给用户。")
                    return
